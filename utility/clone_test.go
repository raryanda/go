...
func TestCopyFromStructToSlice(t *testing.T) {
	...
	if err := utility.Clone(employees, &user); err != nil && len(employees) != 0 {
		t.Errorf("Copy to unaddressable value should get error")
	}

	if utility.Clone(&employees, &user); len(employees) != 1 {
	...
	employees2 := &[]Employee{}
	if utility.Clone(&employees2, user); len(*employees2) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees3 := []*Employee{}
	if utility.Clone(&employees3, user); len(employees3) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if utility.Clone(&employees4, user); len(*employees4) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	...
	if utility.Clone(&employees, users); len(employees) != 2 {
	...
	employees2 := &[]Employee{}
	if utility.Clone(&employees2, &users); len(*employees2) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if utility.Clone(&employees3, users); len(employees3) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if utility.Clone(&employees4, users); len(*employees4) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
