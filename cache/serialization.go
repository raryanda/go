...
func Serialize(value interface{}) ([]byte, error) {
	...
	if err := encoder.Encode(value); err != nil {
		log.Printf("Serialize: gob encoding failed, value: %v, error: %s", value, err)
		return nil, err
	}
	return b.Bytes(), nil
}
...
func Deserialize(byt []byte, ptr interface{}) (err error) {
	...
	if v := reflect.ValueOf(ptr); v.Kind() == reflect.Ptr {
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			if err != nil {
				log.Printf("Deserialize: failed to parse int, value: %s, error: %s", string(byt), err)
			} else {
				p.SetInt(i)
			}
			return
		...
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
			...
			if err != nil {
				log.Printf("Deserialize: failed to parse uint, value: %s, error: %s", string(byt), err)
			} else {
				p.SetUint(i)
			}
			return
	...
	if err = decoder.Decode(ptr); err != nil {
		log.Printf("Deserialize: glob decoding failed, error: %s", err)
		return
	}
	return
}
